...
int pacman_build(alpm_list_t *targets) {
	alpm_list_t *i;
	if (!targets) {
		pm_printf(ALPM_LOG_ERROR, _("no targets specified (use -h for help)\n"));
		return 1;
	...
	for (i = targets; i; i = alpm_list_next(i)) {
		const char *package_name = i->data;

		// Step 1: Fetch PKGBUILD from AUR
		char aur_url[256];
		snprintf(aur_url, sizeof(aur_url), "https://aur.archlinux.org/%s.git", package_name);
		printf("Cloning %s from AUR...\n", package_name);

		char clone_command[512];
		snprintf(clone_command, sizeof(clone_command), "git clone %s", aur_url);
		...
		snprintf(build_command, sizeof(build_command), "cd %s && makepkg -si", package_name);
